...
phillyapi = {
    options: {
    	query : {
            permits:        "SELECT * FROM LI_PERMITS WHERE addresskey IN ('%s') ORDER BY permittype",
            licenses:       "SELECT * FROM LI_BUSINESS_LICENSES WHERE upper(street_address) LIKE CONCAT('%%', upper('%s'), '%%') ORDER BY licensetype",
    ...
    printOwner: function ( temp ) {
        $.each( temp, function(){
            if( ! this.length ) {
               if( DEBUG ) console.log({ owner: this.ownername, legalname: this.legalname, legalfirstname: this.legalfirstname, legallastname: this.legallastname });
            } else {
