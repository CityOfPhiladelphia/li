...
phillyapi = {
  options: {
    query: {
      permits: "SELECT * FROM PERMITS WHERE addressobjectid IN ('%s') ORDER BY permittype", // I used the IN sentence because there could be multiple address key separated by comma (,)
      licenses: "SELECT * FROM BUSINESS_LICENSES WHERE addressobjectid IN ('%s') ORDER BY licensetype",
      violations: "SELECT * FROM VIOLATIONS WHERE addressobjectid IN ('%s') ORDER BY casenumber DESC",
      appeals: "SELECT * FROM APPEALS WHERE addressobjectid IN ('%s') ORDER BY scheduleddate DESC",

      permitsdetail: "SELECT * FROM PERMITS WHERE permitnumber = '%s' AND addressobjectid IN ('%s')",
      licencesdetail: "SELECT * FROM BUSINESS_LICENSES WHERE licensenum = '%s'",
      violationsdetail: "SELECT *  FROM VIOLATIONS WHERE casenumber = '%s' AND addressobjectid IN ('%s')  ORDER BY violationdate DESC",
      appealdetail: "SELECT * FROM APPEALS WHERE internaljobid = '%s' AND addressobjectid IN ('%s') ORDER BY decisiondate DESC LIMIT 1 OFFSET 0",
      desitionshistory: "SELECT * FROM BOARD_DECISIONS WHERE internaljobid = '%s' ORDER BY decisiondate DESC",
      courthistory: "SELECT * FROM COURT_APPEALS WHERE appealkey = '%s' ORDER BY courtactiondate DESC",
    ...
    serviceURL: "https://phl.carto.com/api/v2/sql?q=",
    timeout: 20000
  },
    ...
    phillyapi.fetch(url, function (data) {
      if (data.rows && data.rows[0]) {
        for (i = 0; i < data.rows.length; i++) {
          data.rows[i].mostrecentinvestigation = (!_.isEmpty(data.rows[i].mostrecentinvestigation)) ?
            phillyapi.serverDate(data.rows[i].mostrecentinvestigation).toDateString() :
            " - ";

          data.rows[i].casecompleteddate = (!_.isEmpty(data.rows[i].casecompleteddate)) ?
            phillyapi.serverDate(data.rows[i].casecompleteddate).toDateString() :
            "";

          data.rows[i].casecreateddate = (!_.isEmpty(data.rows[i].casecreateddate)) ?
            phillyapi.serverDate(data.rows[i].casecreateddate).toDateString() :
            " - ";
  ...
  sortViolations: function (rows) {
    // APTYPE field does not exists anymore.
  ...
  sortAppeals: function (rows) {
    ...
    $.each(rows, function (index, obj) {
      rows[index].appeal_type_name = phillyapi.getAppealTypeName(obj.applicationtype);
    });

    rows = phillyapi.sort(rows, 'internaljobid');
  ...
  /**
  * Adding two new functions check multiple strings and return the percentage of
  * similarity, Thanks to 
