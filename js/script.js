 ...
 */
var checkDone = {
		'permits' : false, 'licenses' : false, 'violations' : false, 'appeals' : false, check : function(type){
			checkDone[type] = true;
			if(DEBUG) console.log('done', type);
			if(checkDone.permits && checkDone.licenses && checkDone.violations && checkDone.appeals)
			{
				setTimeout(function(){headerInfo.populateNav()}, 250);
				setLoading(false);
			}
		}
	}
		...
		if(cache.summary != matchObj[0]) {
			phillyapi.getAddressInfo(input, function(response){
				...
				try{
					...
					$("[data-role=\"content\"]", page).html(_.template($("#template-header-sumary").html(), {'address': street_address}));
					...
					phillyapi.getSummary(phillyapi.options.prepare(phillyapi.options.query.permits, addresskey.replace(/\,/g, '\',\'')), function(data) {
						...
						$("#permits-data .data", page).html(_.template($("#template-permits").html(), {'permits': data.rows, 'key': addresskey, 'address': raw_input}));
					...
					//Load Licenses
					phillyapi.getSummary(phillyapi.options.prepare(phillyapi.options.query.licenses, street_address), function(data) {
						if(DEBUG) console.log('Licenses', data);
						$("#licenses-data .data", page).html(_.template($("#template-licenses").html(), {'licenses': data.rows, 'key': street_address, 'address': raw_input}));
						if(!_.isEmpty(data.rows)) {
							// jQuery Mobile enhance list we just created
							$("[data-role=\"collapsible\"]", page).collapsible();
					...
					phillyapi.getSummary(phillyapi.options.prepare(phillyapi.options.query.violations, addresskey.replace(/\,/g, '\',\'')), function(data) {
						...
						$("#violations-data .data", page).html(_.template($("#template-violations").html(), {'violations': data.rows, 'key': addresskey, 'address': raw_input}));
						if(!_.isEmpty(data.rows)) {
							// jQuery Mobile enhance list we just created
					...
					phillyapi.getSummary(phillyapi.options.prepare(phillyapi.options.query.appeals, addresskey.replace(/\,/g, '\',\'')), function(data) {
						...
						$("#appeals-data .data", page).html(_.template($("#template-appeals").html(), {'appeals': data.rows, 'key': addresskey, 'address': raw_input}));
						if(!_.isEmpty(data.rows)) {
							// jQuery Mobile enhance list we just created
	...
	permit: function(eventType, matchObj, ui, page, evt) {
		...
		if(cache.details != matchObj[0]) {
			...
			phillyapi.getPermit(matchObj[1], String(matchObj[2]).replace(/\,/g, '\',\''), function(data) {
				...
				// Pass data to template for rendering
				$("[data-role=\"content\"]", page).html(_.template($("#template-details-permit").html(), {data: data.rows[0]}));
	...
	license: function(eventType, matchObj, ui, page, evt) {
		if(cache.details != matchObj[0]) {
			...
			phillyapi.getLicense(matchObj[1], function(data) {
				...
				$("[data-role=\"content\"]", page).html(_.template($("#template-details-license").html(), {data: data.rows[0]}));
				cache.details = matchObj[0];
				setLoading(false);
	...
	_case: function(eventType, matchObj, ui, page, evt) {
		if(cache.details != matchObj[0]) {
			...
			phillyapi.getCase(matchObj[1], String(matchObj[2]).replace(/\,/g, '\',\''), function(data) {
				...
				$("[data-role=\"content\"]", page).html(_.template($("#template-details-case").html(), {data: data.rows}));
	...
	appeal: function( eventType, matchObj, ui, page, evt ) {
		if( cache.details != matchObj[0] ) {
			...
			phillyapi.getAppeal( matchObj[1], String( matchObj[2] ).replace( /\,/g, '\',\'' ), function( data ) {
				...
				$( "[data-role=\"content\"]", page ).html( _.template( $( "#template-details-appeal" ).html(), { data: data.rows } ) );
				...
				if( ! _.isEmpty( data.rows ) ) {
					...
					phillyapi.getDesitionHistory( data.rows.appeal_key, function( desitionHistory ){
						...
						$( ".desition-history-div" ).html( _.template( $( "#template-desition-history" ).html(), { desitionHistory : desitionHistory } ) );
						$( ".desition-history-div [data-role=\"collapsibleset\"]" ).collapsibleset();
					}, function( response ){ if( DEBUG ) console.log( "Desition History Error:", response ); });
					...
					phillyapi.getCourtHistory( data.rows.appeal_key, function( courtHistory ){
						...
						$( ".court-history-div" ).html( _.template( $( "#template-court-history" ).html(), { courtHistory : courtHistory } ) );
						$( ".court-history-div [data-role=\"collapsibleset\"]" ).collapsibleset();
					}, function( response ){ if( DEBUG ) console.log( "Court History Error:", response ); });
				}
...
new $.mobile.Router({
	...
	"#details\\?entity=permits&eid=(\\d*)&key=(.*)&address=(.*)": { handler: "permit", events: "bs" },
	"#details\\?entity=licenses&eid=(\\d*)&key=(.*)&address=(.*)": { handler: "license", events: "bs" },
	"#details\\?entity=violationdetails&eid=(\\d*)&key=(.*)&address=(.*)": { handler: "_case", events: "bs" },
	"#details\\?entity=appeals&eid=(\\d*)&key=(.*)&address=(.*)": { handler: "appeal", events: "bs" }
}, controller );

/*
 * Necessary because v1.1.0 of jQuery Mobile doesn't seem to let you show the loading message during pagebeforeshow
 */
$.mobile.loading( { theme: "b", text: "Loading...", textonly: false} );
